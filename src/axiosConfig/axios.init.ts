...
// 请求拦截器
$.interceptors.request.use((config: any) => { //config 无法注入axios内置config之外的参数， 所以自定义config.showLoading 是无法接受的
	//console.log(config);
...
function handleLoading(showLoading: any){
...
export default {
	...
	delete: (url: string, data: any ={}, config: any = {}) => {
		let mergeConfig: any = {...defaultConfig, ...config};
		handleLoading(mergeConfig.showLoading);
	...
	postByQS: (url: string, data: any ={}, config: any = {}) => {
		config.headers = {
	...
	postUpload: (url: string, data: any ={}, config: any = {}) => {
		config.headers = {
		...
		let param: any = new FormData()  // 创建form对象
