<template>
    <div class="page-index bg-fff h100 over-auto baseUserInfo">
        <div class="header-wrap">
            <van-nav-bar title="个人基本资料" left-text="" right-text="" left-arrow fixed z-index="10" class="header"
        ...
        <div class="content">
            <van-cell-group class="cell-group">
                ...
                <van-popup v-model="showSexPicker" position="bottom">
                    <van-picker show-toolbar :columns="sexColumns"
                                :default-index="setDefaultIndex(sexColumns, form.sexName)"
                                @cancel="showSexPicker = false" @confirm="onSexConfirm"/>
                </van-popup>
                ...
                <van-popup v-model="showMarryPicker" position="bottom">
                    <van-picker show-toolbar :columns="marryColumns" @cancel="showMarryPicker = false"
                ...
                <van-popup v-model="showEduPicker" position="bottom">
                    <van-picker show-toolbar :columns="eduColumns"
                                :default-index="setDefaultIndex(eduColumns, form.eduStateName)"
                                @cancel="showEduPicker = false" @confirm="onEduConfirm"/>
                </van-popup>
                ...
                <van-popup v-model="showPerTypePicker" position="bottom">
                    <van-picker show-toolbar :columns="perTypeColumns"
                                :default-index="setDefaultIndex(perTypeColumns, form.perTypeName)"
                                @cancel="showPerTypePicker = false" @confirm="onPerTypeConfirm"/>
                </van-popup>

                <van-cell title="所属地区" is-link :value="form.area" @click="showAreaPicker = true"/>
                <van-popup v-model="showAreaPicker" position="bottom">
                    <van-area :area-list="areaList" :value="setAreaDefaultValue(form.area, 2)" :columns-num="2"
                              ref="myArea" title="开户地区" @change="onAreaChange" @confirm="onAreaConfirm"
            ...
            <van-dialog v-model="dialog.show" :title="dialog.title" show-cancel-button @closed="onDialogClosed"
                        :beforeClose="beforeClose"><!--@open="onDialogOpen"-->
                ...
                <div class="input-item text-input text-c">
                    ...
                    <van-field v-model="dialog.value" type="password" placeholder="" v-if="dialog.vType==2"
                               class="pass"/>
                </div>
            </van-dialog>
        </div>
        ...
        methods: {
            ...
            setAreaDefaultValue(strValue, columnNum) {	//columnNum 从1开始，第一列，第二列...
                ...
                if (arr.length >= columnNum) {
                    ...
                    //return
                    Object.entries(filterAreaObj).some(([key, value]) => {
                        rKey = key
            ...
            getBaseUserInfo() {
                this.API.getBaseUserInfo().then(({data}) => {
                    ...
                    let {create_time, account, gender, age, qq, email, marriage, education, crowd, province, city} = data
                    let params = {
            ...
            submit() {
                ...
                let {sexName, age, qq, email, marryName, eduStateName, perTypeName, province, city} = this.form
                ...
                //console.log(params)
                this.API.setBaseUserInfo(params).then(({error}) => {
                    if (error.errno == 200) {
                        this.$toast({
