...
<script lang="ts">
	...
	export default class BaseUserInfo extends Vue {
		...
		getFilterNameById(arrayList:any[] = [], filterId: string): string{
			let arr: any[] = arrayList.filter(({value})=>value == filterId)
			return arr.length?arr[0].text:""
		}
		getFilterValueByName(arrayList: any[] = [], filerName: string): string{
			let arr: any[] = arrayList.filter(({text})=>text == filerName)
			return arr.length?arr[0].value:""
		}
		//设置picker的默认选项
		setDefaultIndex(Columns: any[] = [], value: string): string|number{
			let index: string = ""
		...
		//设置area默认值
		setAreaDefaultValue(strValue: string, columnNum: number): string{	//columnNum 从1开始，第一列，第二列...
			if(!strValue) return "";
		...
		parseProvince(province: string): string {
		...
		beforeClose(action: any, done: any): void {
		...
		openDialog(vTypeName: string): void {
			if(vTypeName == 'age'){
		...
		//确定选择开户地区
		onAreaConfirm(pItem: any, indexArray: any): void {
			//console.log(pItem)
