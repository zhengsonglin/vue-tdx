<template>
	<div class="page-index bg-fff h100 over-auto cardBind">
		<div class="header-wrap">
			<van-nav-bar title="绑定银行卡" left-text="" right-text="" left-arrow fixed z-index="10" class="header" @click-left="onClickLeft">
				<template #right>
			...
			<van-cell-group :class="['cell-group', {'finished':isDisabled}]">
				...
				<van-field v-model="form.yzm" type="digit" center clearable label="手机验证码" placeholder="请输入短信验证码" maxlength="6" v-if="!isDisabled">
					<template #button v-if="!isDisabled">
					    <van-button type="primary" @click="sendCode" size="small" :disabled="validCode.isDisabled">{{validCode.btnText}}</van-button>
					</template>
				</van-field>

				<van-field v-model="form.reallyName" label="真实姓名" placeholder="真实姓名" :readonly="isDisabled"/>
				<van-field readonly label="开户银行" :value="form.bankName" placeholder="选择开户银行" @click="showBankPicker = !isDisabled"
				  	:is-link = "!isDisabled" />
				<van-popup v-model="showBankPicker" position="bottom">
				  	<van-picker show-toolbar title="开户行" :columns="columns" @cancel="showBankPicker = false" @confirm="onBankConfirm" />
				</van-popup>
				
				<van-field readonly  label="开户地区" :value="form.area" placeholder="选择开户地区" @click="showAreaPicker = !isDisabled"
				  	:is-link = "!isDisabled" />
				<van-popup v-model="showAreaPicker" position="bottom" >
				    <van-area :area-list="areaList" :columns-num="2" ref="myArea" title="开户地区" @change="onAreaChange" @confirm="onAreaConfirm" @cancel="onAreaCancel"/>
				</van-popup>
				<van-field v-model="searchKey" type="tel" label="开户支行" placeholder="开户支行关键词搜索" :readonly="isDisabled" v-if="!isDisabled">
					<template #button v-if="!isDisabled">
	...
	export default {
		...
		methods: {
	        ...
	        getSubBranch(flag = false){	//flag: true手动搜索，false初始化加载
	        	...
	        	this.API.getSubBranch({Province:FBankProvince, City:FBankCity, BankName:FName, key: this.searchKey}).then((data)=>{
