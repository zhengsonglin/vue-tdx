<template>
    <div class="page-index bg-fff w100 h100 over-auto sortProduct absolute">
        <div class="header-wrap">
            <van-nav-bar title="商品分类" left-text="" right-text="" left-arrow fixed z-index="10" class="header"
                         @click-left="onClickLeft"></van-nav-bar>
        </div>
        <div class="content flex">
            <div class="left-item h100 over-auto">
                <div :class="['nav-item', {active: category== item.id}]" v-for="(item,index) in categoryList"
                     :key="index" @click.stop="changeCategory(item)">
                    {{item.short_name}}
                </div>
            </div>
            <div class="right-item flex-1 h100 over-auto">
                <div class="right-wraper h100">
                    <div class="top-item flex">
                        <div :class="['status-tag flex-1 border-box', {active:statusItem.status == status}]"
                             v-for="(statusItem, num) in productStatus" :key="num"
                             @click.stop="changeQueryStatus(statusItem)">{{statusItem.name}}
                        </div>
                    </div>
                                    ...
                                    <div class="flex bg-fff">
                                        ...
                                        <div class="right-content flex-1">
                                            ...
                                            <div class="row-3 over-hidden">
                                                <div class="left fl">
                                                    <span class="title">垫付:</span>
                                                    <p class="red inline-block">￥<span
                                                            class="price bold">{{item.price}}</span></p>
                                                </div>
                                                <div class="right fr" v-if="status ==null">
                                                    <span class="btn inline-block c-fff  text-c"
                                                          @click="toProductDetail(item)">马上抢</span>
                                                </div>
                                            </div>
                                            <div class="row-4 relative">
                                                <!--mpvue不支持在template中调用methods方法-->
                                                <div class="bar_box h100"
                                                     :style="{color:'#666',width:item.widthPercent}"></div>
                                                <div class="bar_word w100 h100 c-fff absolute border-box">
                                                    剩余{{item.task_count - item.order_count}}件/共{{item.task_count}}件
                                                    {{item.textPercent }}
                                                </div>
