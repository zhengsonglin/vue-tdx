...
export default class SkillTaskDetail extends Vue {
  ...
  public getSkillTaskDetail(): void {
    
    this.API.getSkillTaskDetail({fid: this.fid}).then((data: any) => {
  ...
  public startTask(): void {
    this.API.startTask({fid: this.fid }).then((data: any) => {
  ...
  public handleBuy(): void {
    ...
    this.API.getSkillTask({fid: this.fid, Mark: 'M'}).then((data: any) => {
      if (data.ErrorCode == 100) {
              this.$dialog.alert({
                title: '恭喜你',
  ...
  public AddYd(): void {
      let tt: RegExp = /^\d+$/g,
        days: string = this.reservationDay;
      if (!tt.test(days)) {
          this.$notify('天数输入错误！');
          return;
      }
      this.showReservation = false
      this.API.reservations({ fid: this.fid, days }).then((data: any) => {
        if (data.ErrorCode == 100) {
          this.$toast({
